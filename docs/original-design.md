...
| 항목 | 내용 |
|---|---|
| 처리 주체 | `qa-pipeline-verifier` 서브에이전트 |
| 관련 스킬 | 전체 스킬 |
| 입력 | 구현된 프로젝트 전체 |
| 출력 | QA 결과 문서 |
| 코드 처리 | 실제 CLI 실행, 실제 DuckDuckGo 네트워크 요청, 출력 파일 확인 |
| LLM 판단 | QA 결과 해석, 문제 원인 분류 |
| 성공 기준 | 실제 사용자 실행 방식으로 테스트 입력 → 결과 파일 생성까지 성공 |
| 검증 방법 | 사람 검토 + LLM 자기 검증 + 규칙 기반 검증 |
| 실패 시 처리 | 구현 담당 에이전트에게 수정 요청, 수정 후 동일 파이프라인 재실행 |
